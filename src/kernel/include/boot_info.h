 ...
 *  O bloco vive em 0x60000 e segue o layout de memory_boot_info_t do stage2.
 *  @author Gabriel-lima
 *  @date 2026-04-18
...
#define BOOT_INFO_ADDR 0x60000 // Endereco onde o stage2 escreve o bloco de boot info (layout binario identico a memory_boot_info_t).
#define E820_MAP_ADDR 0x61000 // Endereco onde o stage2 escreve o mapa de memoria E820 (array de e820_entry_t, layout binario identico a memory_map_t).

/** @brief Converte um far pointer OpenWatcom (segment:offset em uint32_t) para endereco flat.
...
/** @brief Entrada do mapa de memoria E820. Layout identico ao do stage2.
 ...
 *  @param ext_attrs Atributos estendidos (usados para RAM, bit 0 = cacheavel).
 */
...
/** @brief Alocador bump do stage2 (layout binario identico).
...
/** @brief Bloco de boot info escrito pelo stage2 em 0x60000.
 *  @details Os campos far pointer sao armazenados como uint32_t (segment<<16 | offset).
...
/** @brief Retorna ponteiro direto para o bloco de boot info.
 *  @note O bloco é lido diretamente do endereço fixo 0x60000.
 *  @return Ponteiro para boot_info_raw_t; layout binário idêntico ao de memory_boot_info_t do stage2.
 */
static inline const boot_info_raw_t *boot_info_get(void) {
    return (const boot_info_raw_t *)BOOT_INFO_ADDR;
}

/** @brief Retorna ponteiro flat para o array de entradas E820.
 *  @note Acessa o far pointer armazenado no bloco de boot info.
 *  @return Ponteiro flat para e820_entry_t[]; layout binário idêntico ao de memory_map_t do stage2.
 */
